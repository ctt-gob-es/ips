...
import es.map.ipsg.exception.IpsException;
...
public class ValidarCertificado extends AbstractAFirmaClienteWS {
	...
	/** La constante STRING_MENSAJE_ENVIADO. */
	private static final String STRING_MENSAJE_ENVIADO = "mensaje enviado: ";
	
	/** La constante STRING_MENSAJE_RECIBIDO. */
	private static final String STRING_MENSAJE_RECIBIDO = "mensaje recibido: ";
	
	/** La constante STRING_NO_SE_PUDO_VALIDAR_CERTIFICADO. */
	private static final String STRING_NO_SE_PUDO_VALIDAR_CERTIFICADO = "No se pudo validar el certificado";
	...
	public VerificacionResult validar(ParamCert param) {
		...
		try {
			...
			if (logger.isDebugEnabled()) {
				logger.debug(STRING_MENSAJE_ENVIADO + message);
			}
			
			String strResponse = (String) call.invoke( new Object[] { message } );
			
			if (logger.isDebugEnabled()) {
				logger.debug(STRING_MENSAJE_RECIBIDO + strResponse);
			}
			...
			throw new ApplicationException(STRING_NO_SE_PUDO_VALIDAR_CERTIFICADO, e);
		}
	 ...
	 * @return el verificacion result
	 */
	public VerificacionResult almacenar(ParamCert param) {
		...
		try {
		    ...
		    call.setTargetEndpointAddress( new java.net.URL(endpoint) );
		    call.setOperationName(new QName(endpoint, "AlmacenarDocumento" ));
			...
			logger.error("Error- No se pudo validar el certificado",e);
			throw new ApplicationException(STRING_NO_SE_PUDO_VALIDAR_CERTIFICADO);
		}
	...
	public String validar2(ParamCert param) {
		...
		try {
			...
			logger.info(STRING_MENSAJE_RECIBIDO + strResponse);			
