...
/**
 * El Class BuscarSolicitudPresencialSpring.
 *
 * @author amartinl
 */
public class BuscarSolicitudPresencialSpring extends AbstractSpring {
	...
	/** La constante logger. */
	private static final Logger logger = Logger.getLogger(BuscarSolicitudPresencialSpring.class);
	...
	/**
	 * Crea una nueva buscar solicitud presencial spring.
	 */
	public BuscarSolicitudPresencialSpring() {
		try {
				...
				logger.error("Error BuscarSolicitudPresencialSpring - cargar datos ", e);
		}		
	}
	...
	protected String doExecute(SpringForm form) throws Exception {
		getLogger().debug("BuscarSolicitudPresencialSpring -start");
	...
	try{
		...
		try{
			...
			logger.error("Error BuscarSolicitudPresencialSpring - recuperar usuarioSesion "+ sUsernameLogin, e);
		...
		try{
			...
			logger.error("Error BuscarSolicitudPresencialSpring - numRegistros: "+ numReg, e);
		}
		...
		/*INI-TRA042*/
		SolComunPresencialesViewQuery solicitudQuery = crearQuerySolicitudPresencial(formulario,paginaActual,numReg,listaCentrosGestores);
		/*FIN-TRA042*/
		...
		if(STRING_BUSCAR.equals(busqueda) || "Ordenar".equals(busqueda) || "Paginar".equals(busqueda) || "Verificar".equals(busqueda)
			...
			sVieneMenu = "N";
			lSolicitudBean = solicitudesRegistradasManager.buscarSolicitudesPresencialesVista(solicitudQuery);
		...
		if(numReg != null && !"".equals(numReg)){
			...
			numRegistros = Integer.parseInt(numReg);
				...
				logger.error("Error BuscarSolicitudPresencialSpring -  paginacion - numRegistros: "+ numRegistros, e);
		...
		getLogger().debug("BuscarSolicitudPresencialSpring -end");
		
	}catch(Exception eGenerico){
		logger.error("Error BuscarSolicitudPresencialSpring - doExecute: ",eGenerico);
	 ...
	 */
	private void comprobarMarcarTodos(BuscarSolicitudesPresencialesForm formulario) {
	...
	private String cargarCampos(String campo) {
		...
		try{
			...
			logger.error("Error BuscarSolicitudPresencialSpring - cargarcampos - campo: "+ codCampo, e);
		}
...
/*	private SolicitudComunQuery crearQuerySolicitud (BuscarSolicitudesPresencialesForm formulario,String paginaActual,String numReg)
	{
		...
		if(numReg != null && !"".equals(numReg)){
			...
			numRegistros = Integer.parseInt(numReg);
				...
				logger.error("Error BuscarSolicitudPresencialSpring -  paginacion Query - numRegistros: "+ numRegistros, e);
				numRegistros = Integer.parseInt(Constantes.NUMERO_REGISTROS_PAGINA);
			}
		}else{
			try{
		...
		if (sFechaDesde != null && !sFechaDesde.equals("")){
			try
			{
				...
				if(accionAlta!=null && accionAlta.equals("Modificar")){
					solicitudQuery.setFechaUltActualizacionMin(dFechaDesde);
				}else{
				...
				logger.error("Error BuscarSolicitudPresencialSpring - parsear fechaDesde: "+ sFechaDesde, e);
			}
		...
		if (sFechaHasta != null && !sFechaHasta.equals("")){
			try
			{
				...
				logger.error("Error BuscarSolicitudPresencialSpring - parsear fechaHasta: "+ sFechaHasta, e);
			}
		}
		
//		if (solExencion != null && !solExencion.equals("")) {
//			//solicitudQuery.setSolExencion(solExencion.charAt(0)); esto estaba comentado
//		}
		...
		solicitudQuery.addOrder(SolicitudComunQuery.IDSOLICITUD, OrderType.DESC);
 ...
 */
 /*INI-TRA042*/
private SolComunPresencialesViewQuery crearQuerySolicitudPresencial (BuscarSolicitudesPresencialesForm formulario,String paginaActual,String numReg, List<CentroGestorBean> listaCentrosGestores)
	{
		...
		if(iIdModelo!=null && iIdModelo.intValue()!=0){
			...
			centroGestorQuery.setModelo(modeloQuery);
			convocatoriaQuery.setModelo(modeloQuery);
			solicitudQuery.setIdModelo(iIdModelo.shortValue());
		}
		// TODO PRESENCIALES MULTI-MODELO
