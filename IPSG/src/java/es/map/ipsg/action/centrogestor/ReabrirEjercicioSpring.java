 ...
 * @author amartinl
 */
public class ReabrirEjercicioSpring extends AbstractSpring {
	...
	/** La constante logger. */
	private static final Logger logger = Logger.getLogger(ReabrirEjercicioSpring.class);
	...
	public ReabrirEjercicioSpring() {
		try {
			...
			logger.error("Error ReabrirEjercicioSpring()",e );
		}
	}
	...
	try{
		...
		if(idCentroGestor != null){//Dar de Baja el Centro Gestor
			...
			try{
				...
				logger.error("Error reabrirEjerciciorSpring - recuperarUsuarioSesion",e );
			...
			if (bCierraEjercicio){
				...
				//Actualizamos el ejercicio quitándole un año
				...
				desactivaEstadoConvocatoria(centroGestorBean);
				UsuarioBean usuarioBean = (UsuarioBean) getRequest().getSession().getAttribute("usuario");
				...
				return "errorReabrirCentroGestor";
			}
		...
		logger.error("Error reabrirEjercicioSpring -doExecute: ",eGenerico);
...
public void generarRegistroLogGenerico(String username, Long idCentroGestor){
		...
		logGenericoBean.setTipoOperacion(Constantes.OPERACION_REAPERTURA_EJERCICIO);
		logGenericoBean.setDetalleOperacion(RESOURCE_BUNDLE.getString("field.centroGestor.detalleOperacionReabrirEjercicio") + " " + idCentroGestor.toString());
	 ...
	 */
	//Comprueba que el Centro Gestor no tiene asociadas Convocatorias
...
/**
 * Desactiva estado convocatoria.
 *
 * @param centroGestorBean el centro gestor bean
 */
//Función para cerrar el estado de las convocatorias pertenecientes al ejercicio
private void desactivaEstadoConvocatoria(CentroGestorBean centroGestorBean){
