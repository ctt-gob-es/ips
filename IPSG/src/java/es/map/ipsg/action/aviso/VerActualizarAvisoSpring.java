...
/**
 * El Class VerActualizarAvisoSpring.
 */
public class VerActualizarAvisoSpring extends AbstractSpring {
	...
	/** La constante logger. */
	private static final Logger logger = Logger.getLogger(VerActualizarAvisoSpring.class);
	...
	/**
	 * Crea una nueva ver actualizar aviso spring.
	 */
	public VerActualizarAvisoSpring() {
		try {
			...
			logger.warn(e);
			logger.error("Error VerActualizarAvisoSpring():",e );
		}
	}
	...
	protected String doExecute(SpringForm form) throws Exception {
		...
		//****************************************************************** 
		getLogger().debug("VerActualizarAvisoSpring -start");
	try{
		...
		AvisoBean avisoBean;
		Integer idAviso;
		idAviso=formulario.getIdAviso();
		avisoBean=avisoManager.obtenerAviso(idAviso);
		
		formulario.setFechaInicio( formatoDelTexto.format(avisoBean.getFechaInicio()));
		formulario.setFechaFin(formatoDelTexto.format(avisoBean.getFechaFin()));
		formulario.setTexto(avisoBean.getTexto());
		formulario.setIdEstadoAviso(avisoBean.getIdEstadoAviso());
		formulario.setIdAviso(idAviso);
		
		
		setRequestAttribute("estados", estadosList);

		getLogger().debug("VerActualizarAvisoSpring -end");
		
	}catch(Exception eGenerico){
		logger.error("Error VerActualizarAviso - doExecute: "+ eGenerico);
