	...
	var	yearNow	 = today.getYear();if (!ie) { yearNow += 1900;	}
...
//---------------------------------------------TRADUCCION EN ESPAÑOL-----------------------------------------------------------------------------
	...
	function selectYear(selectObject) {
		nYear = selectObject.options[selectObject.selectedIndex].value;		
	...
	function constructYear(tipoCalendario) {
		var ini,fin;
		popDownMonth();
		sHTML =	"";
...
//CAMBIA el siguiente for por este " for	(i=yearNow; i<=(yearNow+4); i++) { " si quieres que los años empizen en el actual en adelante		
			for	(var i=ini; i>=fin; i--) {		
				...
				sHTML += "<option value="+i+" id='y" + sName + "' onmouseover='this.style.backgroundColor=\"#FFCC99\"' onmouseout='this.style.backgroundColor=\"#FFFFDD\"' style='cursor:pointer'>&nbsp;" + sName + "&nbsp;</option>";
			}
			sHTML += "";
			document.getElementById("selectYear").innerHTML = "<select id='selectAnyo' width=44 style='font-family:arial; font-size:11px; border-width:1; border-style:solid; border-color: #A0A0A0; background-color= #FFFFDD' size=5 onchange='selectYear(this)' onclick='event.cancelBubble=true' onmouseover='clearTimeout(timeoutID2)' onmouseout='clearTimeout(timeoutID2);timeoutID2=setTimeout(\"popDownYear()\",100)'>"+sHTML+"</select>";
	...
	function popUpYear(tipoCalendario) {
		var	leftOffset;
		constructYear(tipoCalendario);		
...
//------------------------------------TERMINAR LA FUNCION EN 0 PARA QUE INICIE EN EL DIA ACTUAL EN LA SIGUIENTE LINEA----------------------------------------------
	...
	function popUpCalendar(ctl,	ctl2, format, tipoCalendario) {
		var	leftpos=-190;
		var	toppos=-10;
