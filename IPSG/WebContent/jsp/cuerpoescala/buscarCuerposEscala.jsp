...
function irPagina(pagTotales)
{
	...
	document.forms[0].action = "../spring/buscarCuerposEscala";
}
...
function alta(){
	...
	window.location.href = "../spring/verAltaCuerposEscala";
...
function comprobarRegistros(){
	...
	window.location.href = "../spring/buscarCuerposEscala?paginaActual="+pagina +"&paginasTotales="+numeroPagina +"&llamada=Buscar"+"&numRegistro="+numRegistro;
...
<h1 class="pae-title"> <spring:message code="field.cuerpoEscala.titulo"/>  </h1>
...
<div data-function="accordion-box" class="pae-box">
	<div class="pae-box__header">
		<h3 class="pae-box__header--title">
			<spring:message code="field.textosIntroduccion.cuerpoEscala"/>
		</h3>
	</div>
...
<form:form modelAttribute="cuerpoEscalaForm" action="/IPSG/spring/buscarCuerposEscala" id="formPadre" method="post">
...
<!-- 					<input type="image" src="../images/lupa.png" alt="Buscar Centro Gestor" onclick="return openWindowCentroGestor()"> -->
	...
	<logic:present name="cuerpoEscala" scope="request">	
		...
		<table id="paetable" class="pae-table pae-table--margin">
								...
								<tbody>
									<tr>											
										<th>
											<div class="titulo_izq_tabla">									
												<span class="pae-table__txt-title">
													<spring:message code="field.categoriaMay"/>
												</span>	
											</div>
										</th>
										<th>											
			...
			<logic:iterate id="registro" name="cuerpoEscala" >
				...
				<tr class="pae-table__row" name="row">
					<td data-content="" data-col="col5" data-function="fc-collapse-table" class="pae-table__cell-body w10">					
						<span class="pae-table__txt-base pae-table__span-head">
							<a href="${pageContext.request.contextPath}/spring/verModificarCuerposEscala?id=${registro.id}" style="color: #c33400;">
					...
					<td data-content="" data-col="col3" data-function="fc-collapse-table" class="pae-table__cell-body">
						<span class="pae-table__txt-base pae-table__span-head">
							<bean:write name="registro" property="desCategoria" />
						</span>
					</td>
					<td data-content="" data-col="col3" data-function="fc-collapse-table" class="pae-table__cell-body">
						<span class="pae-table__txt-base pae-table__span-head">
					...
					<td data-content="" data-col="col3" data-function="fc-collapse-table" class="pae-table__cell-body">						
						<span class="pae-table__txt-base pae-table__span-head">
							<a href="${pageContext.request.contextPath}/spring/eliminarCuerposEscala?id=${registro.id}" 
...
<%-- 					<spring:message code="field.Pagina"/>&nbsp; --%>
				...
				<%if(paginaActual != 1) { %>
					<a href="buscarCuerposEscala?paginaActual=<%=paginaActual-1%>&paginasTotales=<%=numeroPagina %>&llamada=Paginar" >&laquo;</a>
				<% } else{  %>	
				...
				<% } 
					...
					{	
						for(int x = 1 ;x<numeroPagina+1; x++){
							String enlacePagina = "buscarCuerposEscala?paginaActual="
								+ x
								+ "&paginasTotales="
								+ numeroPagina
								+ "&llamada=" + paginar;
					...
					<%if(paginaActual != numeroPagina) { %>
						<a href="buscarCuerposEscala?paginaActual=<%=paginaActual+1%>&paginasTotales=<%=numeroPagina %>&llamada=Paginar" >&raquo;</a>
					<% } else{  %>	
			 			<a style="text-decoration: none;cursor: default;background-color:#FFFFFF"></a> 
					<% }  %> 
