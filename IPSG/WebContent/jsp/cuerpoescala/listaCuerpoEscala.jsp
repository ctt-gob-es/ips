...
function cerrar(cuerpo, desc){
	window.opener.document.forms[1].elements["<%=parametro%>"].value = cuerpo;
...
function comprobarRegistros(){
	...
	window.location.href = "../spring/listarCuerpoEscala?paginaActual="+pagina +"&paginasTotales="+numeroPagina +"&llamada=Buscar&centro="+centroGestor +"&numRegistro="+numRegistro+ "&parametro="+param+ "&parametro2="+param2;
}
...
<html>
	...
	<body style="margin-left: 0.4em;" class="background-color-white-ip">
		...
		<form:form modelAttribute="cuerpoEscalaForm" action="/IPSG/spring/listarCuerpoEscala" id="formHijo" name="cuerpoEscalaForm" >
			<div class="pae-layout__item">		
				...
				<h1 class="pae-title2"><spring:message code="field.cuerpoEsclaa.listarCuerpoEscala" /></h1>
				...
				<jsp:useBean id="cuerpoEscala" scope="request"
					class="java.util.ArrayList" />
				<%
					int i = 0;
				%>
				<logic:present name="cuerpoEscala" scope="request">
					<bean:size id="numResultados" name="cuerpoEscala" scope="request" />
					<logic:greaterThan name="numResultados" value="0">
						<table class="pae-table pae-table--margin">
								...
								<tbody class="pae-table__body">
									<c:forEach var="registro" items="${cuerpoEscala }">
										...
										onclick="javascript:cerrar('<bean:write name="registro" property="id" />','<bean:write name="registro" property="descripcion" />')" />  <font color="#c33400"><bean:write name="registro" property="id" /></font> <a/>
				               					</span>
				               				</td>
							...
							<div class="capaPaginacion">
								...
								<%if(paginaActual != 1) { %>
									<a href="listarCuerpoEscala?paginaActual=<%=paginaActual-1%>&paginasTotales=<%=numeroPagina %>&llamada=Paginar&parametro=${parametro}&parametro2=${parametro2}&centro=${centro}">&laquo;</a>
								<% } else{  %>	
							...
							{	
								for(int x = 1 ;x<numeroPagina+1; x++){
									String enlacePagina = new String();
									if(centro != null && !centro.equals(""))
									{	
										enlacePagina = "listarCuerpoEscala?paginaActual="
											+ x
											+ "&paginasTotales="
											+ numeroPagina
											+ "&llamada=" + paginar+ "&parametro="+parametro + "&parametro2="+parametro2+ "&centro="+centro+ "&numRegistro="+numRegistro;
									}
									else
									{	
										enlacePagina = "listarCuerpoEscala?paginaActual="
							...
							<%if(paginaActual != numeroPagina) { %>
								<a href="listarCuerpoEscala?paginaActual=<%=paginaActual+1%>&paginasTotales=<%=numeroPagina %>&llamada=Paginar&parametro=${parametro}&parametro2=${parametro2}&centro=${centro}">&raquo;</a>
							<% } else{  %>	
			 					<a style="text-decoration: none;cursor: default;background-color:#FFFFFF"></a> 
							<% }  %> 
								...
								<div id="rellenarCampos" class="pae-alertbox pae-alertbox--warning">
									...
									<ul>
										<li>
											...
											code="field.cuerpoEscala.error" /></p>
										</li>
									</ul>
								</div>
							</div>
