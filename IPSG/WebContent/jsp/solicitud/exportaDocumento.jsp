...
<%
	Boolean error =(Boolean) request.getAttribute("cargado");
%>
...
function exporta(){
	...
	if(bMarcado)
	{
		document.forms[0].action="../spring/descargarDocumentoZip?ent=Solicitudes";
		...
		setTimeout(function(){
			if(<%=error%>){
				document.getElementById("mensajeExporta").style.display="none";
			}
		},1000,"JavaScript");
		return bMarcado;
	} else
...
<div class="pae-body2" style="overflow-y:hidden">
	<form:form modelAttribute="buscarSolicitudesForm" action="/IPSG/spring/descargarDocumentoZip" id="formPadre">
		<div class="pd-top-3">	
			<div class="pae-layout__item">	
				...
				<form:hidden path="idTipo" id="idTipo" />
				...
				<h1 class="pae-title2"><spring:message code="field.exporta.titulo"/></h1>
						...
						<fieldset>
							...
							<div class="filaBtn">
								<div class="pae-box-buttons">
									<input type="submit" name="submit" value="Exportar" id="botonExportar" onclick="if(!exporta()){return false;}" class="pae-buttons pae-buttons--default pae-buttons--default-modal pae-buttons--medium pae-buttons--download outline pae-separation" style=""/>			
									<input type="button" value="Cerrar" name="submit" id="botonCerrar" onclick="javascript: window.close()" class="pae-buttons pae-buttons--default pae-buttons--default-modal pae-buttons--medium pae-buttons--download outline pae-separation" style=""/>
								</div>
							</div>
							<%-- Boton --%>
							<br><br>
