...
function ordena(value,numPag){
	...
	document.forms[1].action = "../spring/buscarUsuariosAplicacion";
}

function alta(){
	document.getElementById("submit").value="Alta";
	window.location.href = "../spring/verAltaUsuarioAplicacion";
...
function irPagina(pagTotales)
{
	...
	document.forms[0].action = "../spring/buscarUsuariosAplicacion";
	document.forms[1].action = "../spring/buscarUsuariosAplicacion";
}
...
function comprobarBusqueda(){
	...
	window.location.href = "../spring/buscarUsuariosAplicacion?paginaActual="+pagina +"&paginasTotales="+numeroPagina +"&llamada=Buscar"+"&numRegistro="+numRegistro; 
}
...
function comprobarRegistros(){
	...
	pagina = document.getElementById("paginaActual").value;
	numRegistro = document.getElementById("numRegistro").value;
	window.location.href = "../spring/buscarUsuariosAplicacion?paginaActual="+pagina +"&paginasTotales="+numeroPagina +"&numRegistro="+numRegistro+"&submit=Buscar"; 
}
...
<%@page import="es.map.ips.model.UsuarioAplicacion"%>
<%@page import="es.map.ipsg.bean.UsuarioAplicacionBean"%>
<%@page import="es.map.ipsg.form.UsuariosAplicacionForm"%>
<html>
...
<div class="capaAll">
	...
	<h1 class="pae-title"> <spring:message code="field.usuariosaplicacion.titulo"/> </h1>
...
<div data-function="accordion-box" class="pae-box">   
	<div class="pae-box__header">
		<h3 class="pae-box__header--title">
			<spring:message code="field.textosIntroduccion.usuariosaplicacion"/>
		</h3>
	</div>

<form:form modelAttribute="buscarUsuariosAplicacionForm" action="/IPSG/spring/buscarUsuariosAplicacion" id="formPadre" method="post">
	...
	<fieldset id="formulario">
		 <div class="pae-layout">
			<div class="pae-layout__item pae-u-4/12 pae-u-2/12-lap pae-u-1/1-palm">
				<div class="pae-form__cell">
					<span class="pae-form__label pae-form__span-label">
						<spring:message code="field.usuariosaplicacion.usuario"/>
					</span>
			...
			<div class="pae-layout__item pae-u-4/12 pae-u-2/12-lap pae-u-1/1-palm">
				<div class="pae-form__cell">
					<span class="pae-form__label pae-form__span-label">
						<spring:message code="field.usuariosaplicacion.nombre"/>
					</span>
				<logic:present name="nombre" scope="request"> </logic:present>
				<form:input type="text" path="nombre" class="pae-form__input" id="nombre" maxlength="50"/>
				</div>
			</div>
			<div class="pae-layout__item pae-u-5/12 pae-u-2/12-lap pae-u-1/1-palm">
			<div class="pae-form__cell">
			<span class="pae-form__label pae-form__span-label">
				<spring:message code="field.usuariosaplicacion.descripcion"/>
			</span>
			<logic:present name="descripcion" scope="request"></logic:present>
			<form:input type="text" path="descripcion" class="pae-form__input" id="descripcion" maxlength="50"/>
			</div>
			</div>
	...
	<div class="filaBtn">
		<div class="pae-box-buttons pd-right-1">	
			<input type="submit" value="Buscar" name="buscarUsuariosAplicacion" onclick="busqueda()" class="pae-buttons pae-buttons--default pae-buttons--default-modal pae-buttons--medium pae-buttons--download outline">		
	...
	<logic:present name="usuarioAplicacion" scope="request">
		<bean:size id="numResultados" name="usuarioAplicacion" scope="request"/>
		...
		<table id="paetable" class="pae-table pae-table--margin">
			...
			<logic:iterate id="registro" name="usuarioAplicacion" >
				<tr class="pae-table__row" name="row">
					<td data-content="" data-col="col3" data-function="fc-collapse-table" class="pae-table__cell-body wauto">
						<a href="${pageContext.request.contextPath}/spring/verModificarUsuarioAplicacion?id=${registro.id}">
							<font color="#c33400"><bean:write name="registro" property="id" /></font>
						</a>
					</td>
					<td data-content="" data-col="col3" data-function="fc-collapse-table" class="pae-table__cell-body wauto"><bean:write name="registro" property="usuario" /></td>
					...
					<td data-content="" data-col="col3" data-function="fc-collapse-table" class="pae-table__cell-body wauto"><bean:write name="registro" property="descripcion" /></td>
					<td data-content="" data-col="col3" data-function="fc-collapse-table" class="pae-table__cell-body wauto">
					...
					<td data-content="" data-col="col3" data-function="fc-collapse-table" class="pae-table__cell-body">
						<logic:equal name="registro" property="estado" value="1">
							...
							<a href="${pageContext.request.contextPath}/spring/eliminarUsuarioAplicacion?id=${registro.id}" onclick="return comprobarEliminar()">
		...
		%>
			...
			<div class="capaPaginacion" id="cap">
				...
				<%if(paginaActual != 1) { %>
					<a href="buscarUsuariosAplicacion?paginaActual=<%=paginaActual-1%>&paginasTotales=<%=numeroPagina %>&llamada=Paginar">&laquo;</a>
				<% } else{  %>	
			...
			{	
				for(int x = 1 ;x<numeroPagina+1; x++){
					String enlacePagina = "buscarUsuariosAplicacion?paginaActual="
						+ x
						+ "&paginasTotales="
						+ numeroPagina
						+ "&llamada=" + paginar;
			...
			<%if(paginaActual != numeroPagina) { %>
				<a href="buscarUsuariosAplicacion?paginaActual=<%=paginaActual+1%>&paginasTotales=<%=numeroPagina %>&llamada=Paginar">&raquo;</a>
			<% } else{  %>	
			 <a style="text-decoration: none;cursor: default;background-color:#FFFFFF"></a> 
			<% }  %> 			
			</div>
		...
		<logic:present name="accion" scope="request">
				<logic:lessThan name="numResultados" value="1">
						<div id="error">
							<spring:message code="field.usuariosaplicacion.error"/>
						</div>
					</logic:lessThan>
		 </logic:present>		
	</logic:present>	
