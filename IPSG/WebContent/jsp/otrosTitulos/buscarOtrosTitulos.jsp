...
function irPagina(pagTotales)
{
	...
	document.forms[0].action = "../spring/buscarOtrosTitulos";
}
...
function alta(){
	...
	window.location.href = "../spring/verAltaOtrosTitulos";
...
function comprobarRegistros(){
	...
	window.location.href = "../spring/buscarOtrosTitulos?paginaActual="+pagina +"&paginasTotales="+numeroPagina +"&llamada=Buscar"+"&numRegistro="+numRegistro;
...
<h1 class="pae-title"> <spring:message code="field.otrosTitulos.titulo"/>  </h1>
...
<div data-function="accordion-box" class="pae-box">
	<div class="pae-box__header">
		<h3 class="pae-box__header--title">
			<spring:message code="field.textosIntroduccion.otrosTitulos"/>
		</h3>
	</div>
...
<form:form modelAttribute="otrosTitulosForm" action="/IPSG/spring/buscarOtrosTitulos" id="formPadre" method="post">
	...
	<div class="pae-layout__item pae-u-4/12 pae-u-2/12-lap pae-u-1/1-palm">
	   <div class="pae-form__cell">
		  <span class="pae-form__label pae-form__span-label">
             <spring:message code="field.otrosTitulos.descripcion"/>
          </span>          
...
<!-- 					<input type="image" src="../images/lupa.png" alt="Buscar Centro Gestor" onclick="return openWindowCentroGestor()"> -->
	...
	<logic:present name="otrosTitulos" scope="request">	
		<bean:size id="numResultados" name="otrosTitulos" scope="request"/>
		...
		<table id="paetable" class="pae-table pae-table--margin">
			...
			<logic:iterate id="registro" name="otrosTitulos" >
				...
				<tr class="pae-table__row" name="row">
					<td data-content="" data-col="col5" data-function="fc-collapse-table" class="pae-table__cell-body w10">					
						<span class="pae-table__txt-base pae-table__span-head">
							<a href="${pageContext.request.contextPath}/spring/verModificarOtrosTitulos?id=${registro.id}" style="color: #c33400;">
					...
					<td data-content="" data-col="col3" data-function="fc-collapse-table" class="pae-table__cell-body">						
						<span class="pae-table__txt-base pae-table__span-head">
							<a href="${pageContext.request.contextPath}/spring/eliminarOtrosTitulos?id=${registro.id}" 
...
<%-- 					<spring:message code="field.Pagina"/>&nbsp; --%>
				...
				<%if(paginaActual != 1) { %>
					<a href="buscarOtrosTitulos?paginaActual=<%=paginaActual-1%>&paginasTotales=<%=numeroPagina %>&llamada=Paginar" >&laquo;</a>
				<% } else{  %>	
				...
				<% } 
					...
					{	
						for(int x = 1 ;x<numeroPagina+1; x++){
							String enlacePagina = "buscarOtrosTitulos?paginaActual="
								+ x
								+ "&paginasTotales="
								+ numeroPagina
								+ "&llamada=" + paginar;
					...
					<%if(paginaActual != numeroPagina) { %>
						<a href="buscarOtrosTitulos?paginaActual=<%=paginaActual+1%>&paginasTotales=<%=numeroPagina %>&llamada=Paginar" >&raquo;</a>
					<% } else{  %>	
			 			<a style="text-decoration: none;cursor: default;background-color:#FFFFFF"></a> 
					<% }  %> 
		  ...
		  <logic:present name="accion" scope="request">
				<logic:lessThan name="numResultados" value="1">
						<div id="error">
							<spring:message code="field.otrosTitulos.error"/>
						</div>
					</logic:lessThan>
		 </logic:present>
			</logic:present>
