...
function irPagina(pagTotales)
{
	...
	document.forms[0].action = "../spring/buscarTituloOficial";
}
...
function alta(){
	...
	window.location.href = "../spring/verCrearTituloOficial"; 
}
...
function comprobarRegistros(){
	...
	window.location.href = "../spring/buscarTituloOficial?paginaActual="+pagina +"&paginasTotales="+numeroPagina +"&llamada=Buscar"+"&numRegistro="+numRegistro; 
}
</script>
<html>
...
<div class="capaAll">
	<h1 class="pae-title"> <spring:message code="field.titulo.tituloBuscar"/> </h1>
  ...
  <div data-function="accordion-box" class="pae-box">   
	<div class="pae-box__header">
		<h3 class="pae-box__header--title">
			<spring:message code="field.titulo.explicacion"/>
		</h3>
	</div>
...
<form:form modelAttribute="tituloOficialForm" action="/IPSG/spring/buscarTituloOficial" id="formPadre" method="POST">
...
<div class="pae-layout">
	<div class="pae-layout__item pae-u-4/12 pae-u-2/12-lap pae-u-1/1-palm">
		<div class="pae-form__cell">
			...
			<spring:message code="field.titulo.descripcion.mayus"/>
			</span>
...
%>
	<logic:present name="titulos" scope="request">	
		<bean:size id="numResultados" name="titulos" scope="request"/>
		<logic:greaterThan name="numResultados" value="0">
		<table id="paetable" class="pae-table pae-table--margin">
		<caption class="hiddenAccesible">Tabla de Titulos oficiales</caption>
		<thead class="pae-table__header">
			...
			<logic:iterate id="registro" name="titulos" >
				<tr class="pae-table__row" name="row">
					<td data-content="" data-col="colauto" data-function="fc-collapse-table" class="pae-table__cell-body">						
						<span class="pae-table__txt-base pae-table__span-head">
							<a href="${pageContext.request.contextPath}/spring/verModificarTituloOficial?id=${registro.id}" style="color: #c33400;">
							<bean:write name="registro" property="id" />
						</span>
						</a>
<%-- 						<a href="javascript:submitDetalle('${registro.id}','${pageContext.request.contextPath}/spring/verModificarTituloOficial')" style="color: #c33400;"> --%>
<%-- 							<bean:write name="registro" property="id" /> --%>
<!-- 						</a> -->
					</td>
					<td data-content="" data-col="colauto" data-function="fc-collapse-table" class="pae-table__cell-body">
...
<%-- 					<%if (campoVisibilidad.toString() == "true") {%></td> --%>
<!-- 						<td> -->
<%-- 							<spring:message code="field.general.visible"/> --%>
<!-- 						</td>	 -->
<%-- 					<%} else { %>	 --%>
...
<%-- 					<%} %> --%>
					<td data-content="" data-col="colauto" data-function="fc-collapse-table" class="pae-table__cell-body">						
							  <span class="pae-table__txt-base pae-table__span-head">
							  		<a href="${pageContext.request.contextPath}/spring/eliminarTituloOficial?id=${registro.id}" onclick="return comprobarEliminar();" style="color: #c33400;">
		...
		<br>
			...
			<%if(paginaActual != 1) { %>
			<a href="buscarTituloOficial?paginaActual=<%=paginaActual-1%>&paginasTotales=<%=numeroPagina %>&llamada=Paginar" >&laquo;</a>
			<% } else{  %>	
			...
			{	
				for(int x = 1 ;x<numeroPagina+1; x++){
					String enlacePagina = "buscarTituloOficial?paginaActual="
						+ x
						+ "&paginasTotales="
						+ numeroPagina
						+ "&llamada=" + paginar;
			...
			<%if(paginaActual != numeroPagina) { %>
				<a href="buscarTituloOficial?paginaActual=<%=paginaActual+1%>&paginasTotales=<%=numeroPagina %>&llamada=Paginar" >&raquo;</a>
			<% } else{  %>	
			 <a style="text-decoration: none;cursor: default;background-color:#FFFFFF"></a> 
			<% }  %>
			</div>
		...
		<logic:present name="submit" scope="request">
				<logic:lessThan name="numResultados" value="1">
						<div id="error">
							<spring:message code="field.titulo.error"/>
						</div>
					</logic:lessThan>
		 </logic:present>
