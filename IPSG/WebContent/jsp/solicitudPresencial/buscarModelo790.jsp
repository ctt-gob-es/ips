...
function irPagina(pagTotales)
{
	...
	document.forms[0].action = "../spring/buscarModelo790";
}
...
function ordena(value,numPag){
	...
	document.getElementById("accion").value="Ordenar"; //Añadido
	document.forms[0].action = "../spring/buscarModelo790";
	$("#formPadre").attr("action", "../spring/buscarModelo790"); //Añadido
}
	...
	if(comprobarCheckMarcado()){		
		if (document.getElementById("marcarTodo").value == 'TodoOk') {				
			...
			if(numRegs<=50){
				...
				document.forms[1].action = "../spring/consultarLotesAux";
...
function busqueda(){
	...
	$("#formPadre").attr("action", "../spring/buscarModelo790");
...
function openWindowInformacionLotes(parametro) {
	 if (parametro != "null") {
		...
		ventanaPopup.document.title = 'Consulta de Lotes 790';
	}
}
...
function comprobarRegistros(){
	...
	window.location.href = "../spring/buscarModelo790?paginaActual="+pagina +"&paginasTotales="+numeroPagina +"&numRegistro="+numRegistro; 
}
...
{
	document.forms[1].action = "../spring/buscarModelo790";
...
function openWindowVerFecNacimiento(id){
	ventanaPopup = window.open("../spring/verificarFechaNacimiento?auxiliar=true&idSolicitud="+id, 'miventana', 'width=550, height=470,toolbar=no,directories=no,status=no,linemenubar=no,scrollbars=yes,resizable=no ,modal=yes');
	ventanaPopup.focus();
	return false;
}
function openWindowVerDesempleo(id){
	ventanaPopup = window.open("../spring/consultarDesempleo?auxiliar=true&idSolicitud="+id, 'miventana', 'width=680, height=500,toolbar=no,directories=no,status=no,linemenubar=no,scrollbars=yes,resizable=no ,modal=yes');
	ventanaPopup.focus();
	return false;
}
function openWindowVerFnumerosa(id){
	ventanaPopup = window.open("../spring/consultarFNumerosa?auxiliar=true&idSolicitud="+id, 'miventana', 'width=550, height=470,toolbar=no,directories=no,status=no,linemenubar=no,scrollbars=yes,resizable=no ,modal=yes');
	ventanaPopup.focus();
	return false;
}
function openWindowVerDiscapacidad(id){
	ventanaPopup = window.open("../spring/consultarDiscapacidad?auxiliar=true&idSolicitud="+id, 'miventana', 'width=550, height=470,toolbar=no,directories=no,status=no,linemenubar=no,scrollbars=yes,resizable=no ,modal=yes');
	ventanaPopup.focus();
	return false;
}
	...
	if(comprobarCheckMarcado())
	{
		if (document.getElementById("marcarTodo").value == 'TodoOk') {
			...
			if (numRegs<=50){
				...
				document.getElementById("accion").value="ExportarAuxiliares";
...
function marcarCheckPaginacion(pag,paginasTotales) {
	...
	window.location.href = "../spring/buscarModelo790?paginaActual="+pag +"&paginasTotales="+paginasTotales +"&llamada="+"Paginar" +"&marcarTodo="+marcar;
}
</script>
...
<form:form commandName="buscarModelo790Form" action="/IPSG/spring/buscarModelo790" id="formPadre" method="post">
...
<h1 class="pae-title"><spring:message code="field.solicitudPresencial.solicitudBuscarModelo790"/></h1>   
	...
	<logic:present name="solicitudes" scope="request">	
	   ...
	   ||  sPerfilUsuario.equals(Constantes.ROL_ADMINISTRADOR))//Muestra Exportar y Verificar
		{%>
				...
				{%>
					<input type="submit" name="lotes" value="Consulta lotes" onclick="return consultarLotes();" class="pae-buttons pae-buttons--default pae-buttons--default-modal pae-buttons--medium pae-buttons--download outline pae-separation">					
				<%}%>
...
<!--                			</td> -->
					<c:if test="${registro.motivoReduccion ne null}" >	
               			<td data-content="" data-col="col3" data-function="fc-collapse-table" class="pae-table__cell-body">
						<span class="pae-table__txt-base pae-table__span-head">
							<input type="checkbox" id="checkRegEx" value="true" data-function="checkbox-custom-input" class="pae-form__custom-check" disabled="true" checked>
					...
					<c:if test="${registro.motivoReduccion eq null}" >	
					<td data-content="" data-col="col3" data-function="fc-collapse-table" class="pae-table__cell-body">
						<span class="pae-table__txt-base pae-table__span-head">
							<input type="checkbox" id="checkRegEx" value="false" data-function="checkbox-custom-input" class="pae-form__custom-check" disabled="true">
							<label for="checkRegEx" data-function="checkbox-custom" class="pae-form__custom-check-label pae-form__custom-check-label--option"/>
						</span>
               			</td>
					</c:if>
			...
			<div class="capaPaginacion" id="cap">
				...
				<%if(paginaActual != 1) { %>
					<a href="buscarModelo790?paginaActual=<%=paginaActual-1%>&paginasTotales=<%=numeroPagina %>&llamada=Paginar" >&laquo;</a>
				<% } else{  %>	
			...
			{	
				for(int x = 1 ;x<numeroPagina+1; x++){
					enlacePagina = "buscarModelo790?paginaActual="+x +"&paginasTotales="+numeroPagina +"&llamada="+paginar;
					if(x==paginaActual){%>
			...
			<%if(paginaActual != numeroPagina) { %>
				<a href="buscarModelo790?paginaActual=<%=paginaActual+1%>&paginasTotales=<%=numeroPagina %>&llamada=Paginar" >&raquo;</a>
			<% } else{  %>	
			 <a style="text-decoration: none;cursor: default;background-color:#FFFFFF"></a> 
			<% }  %> 	
			</div>			
