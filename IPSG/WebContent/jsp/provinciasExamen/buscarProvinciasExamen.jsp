...
function irPagina(pagTotales)
{
	...
	document.forms[0].action = "../spring/buscarProvinciasExamen";
}

function alta(){
	document.getElementById("submit").value="Alta";
	window.location.href = "../spring/crearProvinciaExamen?accion=<%=Constantes.OPERACION_ALTA %>"; 
}
...
function comprobarRegistros(){
	...
	window.location.href = "../spring/buscarProvinciasExamen?paginaActual="+pagina +"&paginasTotales="+numeroPagina +"&llamada=Buscar"+"&numRegistro="+numRegistro; 
}
...
<div class="">
 ...
 <h1 class="pae-title"> <spring:message code="field.provinciaExamen.titulo"/> </h1>
...
<div data-function="accordion-box" class="pae-box">
	<div class="pae-box__header">
		<h3 class="pae-box__header--title">
			<spring:message code="field.textosIntroduccion.provinciasExamen"/>
		</h3>
	</div>

<!-- <div style="margin-left:1em; margin-right: 1em;float: left;width: 99%;"> -->

<form:form commandName="buscaProvinciasExamenForm" action="/IPSG/spring/buscarProvinciasExamen" id="formPadre" method="post">
	...
	<jsp:useBean id="provinciasExamen" scope="request" class="java.util.ArrayList"/>
	<%int i=0; %>
	<logic:present name="provinciasExamen" scope="request">	
		<bean:size id="numResultados" name="provinciasExamen" scope="request"/>
		<logic:greaterThan name="numResultados" value="0">
		<table id="paetable" class="pae-table pae-table--margin">
				<caption class="hiddenAccesible">Tabla de Provincias Examen</caption>
			...
			<logic:iterate id="registro" name="provinciasExamen" >
				<tr class="pae-table__row" name="row">
					<td data-content="" data-col="col5" data-function="fc-collapse-table" class="pae-table__cell-body">						
							<span class="pae-table__txt-base pae-table__span-head">
								<a href="${pageContext.request.contextPath}/spring/verModificarProvinciaExamen?registro=${registro.id}" id="id" style="color: #c33400;">
					...
					<td data-content="" data-col="col2" data-function="fc-collapse-table" class="pae-table__cell-body">						
							<span class="pae-table__txt-base pae-table__span-head">
								<a href="${pageContext.request.contextPath}/spring/eliminarProvinciaExamen?registro=${registro.id}" id="id" style="color: #c33400;" onclick="return comprobarEliminar()" >
		...
		<div class="pagination">
			<%if(paginaActual != 1) { %>
					<a href="buscarProvinciasExamen?paginaActual=<%=paginaActual-1%>&paginasTotales=<%=numeroPagina %>&llamada=Paginar">&laquo;</a>
			<% } else{  %>	
			...
			{	
				for(int x = 1 ;x<numeroPagina+1; x++){
					String enlacePagina = "buscarProvinciasExamen?paginaActual="
						+ x
						+ "&paginasTotales="
						+ numeroPagina
						+ "&llamada=" + paginar;
			...
			<%if(paginaActual != numeroPagina) { %>
				<a href="buscarProvinciasExamen?paginaActual=<%=paginaActual+1%>&paginasTotales=<%=numeroPagina %>&llamada=Paginar">&raquo;</a>
			<% } else{  %>	
			 <a style="text-decoration: none;cursor: default;background-color:#FFFFFF"></a> 
			<% }  %>			
			</div>
