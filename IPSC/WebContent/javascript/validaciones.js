...
function motivoReduccion(ctrl) {
	...
	function cambiarChecks(){
		if($("#bank-account").val() != undefined){
			document.getElementById("bank-account").checked = false;
		}
		if($("#pagoEfectivo").val() != undefined){
			document.getElementById("pagoEfectivo").checked = true;
		}
	}
...
function actualizarImporte(idSeleccionado) {
	...
	if (find) {
		...
		if (document.getElementById("importe") != null) {
			for (var i = 1; i <= orgsSelect.length; i++) {
				...
				if (id == idSeleccionado) {
					...
					importeNuevo = Math.round((importeOriginal - descuento) * 100) / 100;
		...
		//En caso de no venir de una página con campo con id motivoRedEx
		//iteramos obteniendo todos los elementos check de exencion.
		if (iteracion.length == 0){
			iteracion = document.getElementsByName("checkbox-exemption");
			var camposExen= false;
			...
			for (var k = 1; k <= iteracion.length; k++) {
				...
				//DISCAPACIDAD - mostrar campos discapacidad ocultar FN
				if(document.getElementById("checkbox-exemption-" + k).checked 
						&& document.getElementById("checkbox-exemption-" + k).value == idSeleccionado 
						&& idSeleccionado == 1){
					camposExen = true;
				...
				}else if(document.getElementById("checkbox-exemption-" + k).checked 
						&&document.getElementById("checkbox-exemption-" + k).value == idSeleccionado 
						&& (idSeleccionado==3 || idSeleccionado ==5)){
					//FAMILIA NUMEROSA  - mostrar campos FN ocultar discapacidad
					camposExen = true;
			...
			//Si no hay check en exencion con campos propios se ocultan.
			if(!camposExen){
				$("#datosDiscapacidad").css("display", "none");
				document.getElementById("comunidadesFNExento").className = "animated fadeInLeft pae-layout__item pae-u-6/12 pae-u-6/12-lap pae-u-1/1-palm pae-hidden pae-margin-top";
			}
...
function validarParteComun() {
	...
	// sean correctos
	if ($("#bank-account").val() != undefined && document.getElementById("bank-account").checked == true) {
