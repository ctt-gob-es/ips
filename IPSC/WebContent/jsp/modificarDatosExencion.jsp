              ...
              <div class="pae-box__body">
                  ...
                  <div class="pae-layout">
                    <div class="pae-layout__item pae-u-4/12 pae-u-4/12-lap pae-u-1/1-palm">
                      <div class="pae-form__cell">
                        ...
                        <jsp:useBean id="motivosCompletos2" scope="request" class="java.util.ArrayList" />
							<%int i = 1;%>
							<logic:iterate id="motivosExencion" name="motivosCompletos2">
							
                          <li class="pae-form__list-item">
                          	<c:if test="${motivosExencion.id ne altaSolicitud.idMotivoReduccion }">
                            ...
                            <c:if test="${motivosExencion.id eq altaSolicitud.idMotivoReduccion }">
                           	 	<input type="checkbox" name="checkbox-exemption" id="checkbox-exemption-<%=i%>" value="<bean:write name="motivosExencion" property = "id"/>" checked="true"
                            	onchange="actualizarImporte(this.value);" data-function="checkbox-custom-input" class="pae-form__custom-check" >
                            </c:if>
                          ...
                          <jsp:useBean id="motivos2" scope="request" class="java.util.ArrayList" />
							<logic:iterate id="motivosExencionIncompletos" name="motivos2">
							
                          <li class="pae-form__list-item">
                         	<c:if test="${motivosExencionIncompletos.id ne altaSolicitud.idMotivoReduccion }">
                            ...
                            <c:if test="${motivosExencionIncompletos.id eq altaSolicitud.idMotivoReduccion }">
                            	<input type="checkbox" name="checkbox-exemption" id="checkbox-exemption-<%=i%>" value="<bean:write name="motivosExencionIncompletos" property = "id"/>"  checked="true"
                            	onchange="actualizarImporte(this.value);" data-function="checkbox-custom-input" class="pae-form__custom-check" >
                            </c:if>
                ...
                <logic:present name="registroSolicitud" scope="request">
					<logic:equal value="SI" name="registroSolicitud" property="aniadirDocumentos">
                      ...
                      <legend class="animated fadeInLeft delay-2s  pae-form__legend-text"><spring:message code="field.convocatoria.documentos"/></legend>
                    ...
                    <%-- Si contiene comunidad Exento INI --%>
                    <c:choose>
					    <c:when test="${not empty altaSolicitud.idComunidadFNExento}">
					        <div class="animated fadeInLeft pae-layout__item pae-u-6/12 pae-u-6/12-lap pae-u-1/1-palm pae-margin-top" id="comunidadesFNExento">
					    </c:when>    
					    <c:otherwise>
					        <div class="animated fadeInLeft delay-2s pae-layout__item pae-u-6/12 pae-u-6/12-lap pae-u-1/1-palm pae-hidden pae-margin-top" id="comunidadesFNExento">
					    </c:otherwise>
					</c:choose>
                    <%-- Si contiene comunidad Exento FIN --%>
                        ...
                        <form:select path="idComunidadFNExento" class="pae-form__input" id="idComunidadFNExento" onchange="tituloRequerido()">
                           	<option value="0" class="animated fadeInLeft delay-2s  pae-form__option"><spring:message code="field.solicitud.selecciona"/></option>
                      	 	<form:options items="${listcomunidadesFN}" itemLabel="descripcion" itemValue="idComunidad" style="text-transform: uppercase;"/>	
                      	 	<c:forEach var="comunidad" items="${listcomunidadesFN}" >				
								<c:choose>
								    <c:when test="${comunidad.idComunidad eq altaSolicitud.idComunidadFNExento || comunidad.idComunidad eq pagoSolicitudForm.idComunidadFNExento}">
								        <option value="${comunidad.idComunidad}" style="text-transform: uppercase;" selected="selected">${comunidad.descripcion}</option>
								    </c:when>    
								    <c:otherwise>
								        <option value="${comunidad.idComunidad}" style="text-transform: uppercase;">${comunidad.descripcion}</option>
								    </c:otherwise>
								</c:choose>
							</c:forEach>	
                      	 </form:select>
                      	 <%-- Si contiene numero titular INI --%>
	                     <c:choose>
						    <c:when test="${not empty altaSolicitud.tituloFanumerosa}">
						        <div id="tituloFN"/>
						    </c:when>    
						    <c:otherwise>
	                     ...
	                     <%-- Si contiene numero titular FIN --%>
                      	 ...
                      	 <input type="text" id="tituloFNExento" name="tituloFNExento" class="pae-form__input" value="${altaSolicitud.tituloFanumerosa}" maxlength="20"/>
                  ...
                  <%@ include file="/jsp/modificarDatosDiscapacidad.jsp" %>
                </fieldset>
              </div>
            </div>
