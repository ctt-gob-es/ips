...
function validarDatosDiscapacidad(error){
	...
	if (discapacidadChecked == true) {		
		...
		if(($("#idComunidadDDExento").prop("disabled") == false) &&
				($("#idComunidadDDExento").val() == null  || $("#idComunidadDDExento").value=="" || document.getElementById("idComunidadDDExento").selectedIndex==0)){
			error=1;
	...
	function mostrarOposicion(){
		if(document.getElementById('ck-accept-access-db').checked){
			document.getElementById('motivoOposicion').style.display = 'block';
			document.getElementById('lblMotivoOposicion').style.display = 'block';
		}else{
			document.getElementById('motivoOposicion').style.display = 'none';
			document.getElementById('lblMotivoOposicion').style.display = 'none';
		}
	}
	function rellenaSolicitud() {
		...
		if(busquedaJustResult[i].consentimiento=='SI') {	//Al cambiar  el check consiento a no consiento, invertimos la logica, con false consiente el acceso
		...
		//checkeo idConsentimientoAEAT
		if(busquedaJustResult[i].idConsentimientoAEAT=='true') {	//Al cambiar  el check consiento a no consiento, invertimos la logica, con false consiente el acceso
...
// 		$("#conforme").prop("checked",true);
	...
	$(document).ready(function(){
 		...
 		/*if($('#idComunidadDDExento') != null && $('#idComunidadDDExento').val() != 0){
 			var valueDD = $('#idComunidadDDExento').val();
 			$('#idComunidadDDExento option[value="'+value+'"]').attr("selected", "selected");
 		}else{
			$('[name="porcentajeDiscapacidad"]').attr("disabled", "true");
			$('#idComunidadDDExento').attr("disabled", "true");
 		}*/

		if($('#idComunidadFNExento') != null && $('#idComunidadFNExento').val() != 0){
			var valueFN = $('#idComunidadFNExento').val();
			$('#idComunidadFNExento option[value="'+valueFN+'"]').attr("selected", "selected");
			if ($('input:checkbox[name=checkbox-exemption]:checked')[0] != null && $('input:checkbox[name=checkbox-exemption]:checked')[0].labels[0].innerText == "F. NUMEROSA ESPECIAL") {
				document.getElementById("formPago").value = "E";
				$("#motivo").val("3");
			} else if ($('input:checkbox[name=checkbox-exemption]:checked')[0] != null && $('input:checkbox[name=checkbox-exemption]:checked')[0].labels[0].innerText == "F. NUMEROSA GENERAL") {
				$("#motivo").val("5");
			}
		}

		if($('#checkbox-exemption-1').is(':checked')){
			actualizarImporte(2);             
			mostrarInformacionExencion($('#checkbox-exemption-1'));         
		}else if($('#checkbox-exemption-2').is(':checked')){
			actualizarImporte(1);             
			mostrarInformacionExencion($('#checkbox-exemption-2'));        
		}else if($('#checkbox-exemption-3').is(':checked')){
			atualizarImporte(3);           
			mostrarInformacionExencion($('#checkbox-exemption-3'));     
 		}else if($('#checkbox-exemption-4').is(':checked')){
 			actualizarImporte(6);      
			mostrarInformacionExencion($('#checkbox-exemption-4'));
		}else if($('#checkbox-exemption-5').is(':checked')){
			actualizarImporte(5);
			mostrarInformacionExencion($('#checkbox-exemption-5'));     
		}     
        
		// Comprobamos si existe posibilidad de modificar el pago
		if($('#pagoInicialModif') != undefined && $('#pagoInicialModif').val() == 'N'){
			//Si no existe posibildiad se deshabilitan los checks de exencion
			//$("input[name = checkbox-exemption]").prop("disabled",true);
			$("input[name = checkbox-exemption]").attr("onchange","actualizarSolicitudPagada(this.value);actualizarImporte(this.value);");
		}
		
	});
	
	function mostrarInformacionExencion(checkbox) {
		var a = $(".pae-form__advisor-tooltip--dynamic"),
			b = $(checkbox).attr("id"), 
			c = $("[for='" + b + "']"), 
			d = $("[for='" + b + "']").attr("id");
			...
			c.hasClass("checked") ? (
				...
				c.addClass("unchecked")) : c.hasClass("unchecked") && (c.removeClass("unchecked"),         
			c.addClass("checked"),         
			"DISCAPACIDAD" === d ? (a.children().remove(),         
			a.show(),         
			a.removeClass().addClass("animated fadeInLeft pae-form__advisor-tooltip pae-form__advisor-tooltip--dynamic pae-form__advisor-tooltip--checkbox-one"),         
			a.append($(".pae-form__advisor-checkbox--1").html())) : "DESEMPLEO" === d ? (a.children().remove(),         
			a.show(),         
			a.removeClass().addClass("animated fadeInLeft pae-form__advisor-tooltip pae-form__advisor-tooltip--dynamic pae-form__advisor-tooltip--checkbox-two"),         
			a.append($(".pae-form__advisor-checkbox--2").html())) : "F. NUMEROSA ESPECIAL" === d ? (a.children().remove(),         
			a.show(),         
			a.removeClass().addClass("animated fadeInLeft pae-form__advisor-tooltip pae-form__advisor-tooltip--dynamic pae-form__advisor-tooltip--checkbox-three"),         
			a.append($(".pae-form__advisor-checkbox--3").html())) : "INCIDENCIA DE PAGO" === d ? (a.children().remove(),         
			a.show(),   
			a.removeClass().addClass("animated fadeInLeft pae-form__advisor-tooltip pae-form__advisor-tooltip--dynamic pae-form__advisor-tooltip--checkbox-four"),         
			a.append($(".pae-form__advisor-checkbox--4").html())) : "F. NUMEROSA GENERAL" === d ? (a.children().remove(),         
			a.show(),         
			a.removeClass().addClass("animated fadeInLeft pae-form__advisor-tooltip pae-form__advisor-tooltip--dynamic pae-form__advisor-tooltip--checkbox-five"),         
			a.append($(".pae-form__advisor-checkbox--5").html())) : "VICTIMA TERRORISMO" === d && (a.children().remove(),         
			a.show(),
			a.removeClass().addClass("animated fadeInLeft pae-form__advisor-tooltip pae-form__advisor-tooltip--dynamic pae-form__advisor-tooltip--checkbox-six"),
			a.append($(".pae-form__advisor-checkbox--6").html())))     
	}; 
...
<form:form action="/IPSC/secure/iniciarSolicitud" modelAttribute="solicitudForm" id="formSolicitud" method="post" name="solicitudForm">
		<input type="hidden" name="pagoInicialModif" id="pagoInicialModif" value="${altaSolicitud.pagoInicialModif }"/>
		...
		<bean:define id="idConvocatoria" name="altaSolicitud" property="idConvocatoria"/>
     	...
     	<form:hidden path="importeOriginal" value="${altaSolicitud.importeOriginal}"/>      	
     	<form:hidden path="importePagado" value="${altaSolicitud.importePagado}"/>   
      	...
      	<form:hidden path="idMotivoReduccion"/>
      	<form:hidden path="importePagado"/>
	 	...
	 	<form:hidden path="motivoOposicion" id="mOposicion"/>
	 	<!-- Fin campos formulario discapacidad -->
      	...
      	<input type="hidden" name="altaSolicitud"  id="idMotivoReduccion"/>
       ...
       <input type="hidden" value="<%=siglasCentroGestor%>" name="siglasCentroGestor"/>
        ...
        <%@ include file="/jsp/modificarDatosSolicitante.jsp" %>
        <%@ include file="/jsp/modificarDatosConvocatoria.jsp" %>
        <%@ include file="/jsp/modificarInformacionAdicional.jsp" %>
