	...
	function actualizarImporte(idSeleccionado) {
		...
		for (var k = 1; k <= iteracion.length; k++) {
			if (!(document.getElementById("checkbox-exemption-" + k)
					...
					.getElementById("checkbox-exemption-" + k).value == idSeleccionado)) {
			...
			for (var j = 1; j <= iteracion.length; j++) {
				if (document.getElementById("checkbox-exemption-" + j).checked) {
					...
					if (idSeleccionado == 3 || idSeleccionado == 5) {
						...
						document.getElementById("comunidadesFNExento").className = "animated fadeInLeft pae-layout__item pae-u-6/12 pae-u-6/12-lap pae-u-1/1-palm pae-margin-top";
						...
						document.getElementById("motivoAdeudo").value = 0;
						document.getElementById("motivoTarjeta").value = 0;
						document.getElementById("comunidadesFNExento").className = "animated fadeInLeft pae-layout__item pae-u-6/12 pae-u-6/12-lap pae-u-1/1-palm pae-hidden pae-margin-top";
						document.getElementById("tituloFN").className = "pae-hidden";
					...
					if (idSeleccionado == 1) {
						...
						$("[for='state'] span").removeClass(
								"pae-form__label--disabled").addClass(
								"pae-form__label--text");
					} else {
						if (document.getElementById("idComunidadDDExento") != null) {
							document.getElementById("idComunidadDDExento").disabled = true;
						}
						$("[for='state'] span").addClass(
								"pae-form__label--disabled").removeClass(
								"pae-form__label--text");
					}
						...
						if (id == idSeleccionado) {
							...
							if (document.getElementById("inputIdFormaAcceso").value != 1) {
								...
								if ($('#importePagado').val() != null) {
									importeNuevo = parseFloat((importeNuevo - $('#importePagado').val())).toFixed(2);
					...
					//Si el importe es 0 o menos que 0. No deben de aparecer los datos bancarios/tarjeta a pagar
					if (document.getElementById("importe").value == 0.00 || document.getElementById("importe").value < 0 ) {
					...
					// Compruebo si hay un importe pagado, de ser asi se resta al importe nuevo
					if ($('#importePagado').val() != null) {
						importeNuevo2 = parseFloat((importeNuevo2 - $('#importePagado').val())).toFixed(2);
						$('#mensajePagoModificado').hide();
						$("#importeHTML")[0].innerText = importeNuevo2;
						$("#importe").val(importeNuevo2);
						$("#pagoInicialModif").val("S");
						$("#pagoInicialModifPago").val("S");
						//En la modificacioncon todo pagado, si el importe a pagar es 0 se oculta.
						//Sino salta una validación que obliga a informar datos de pago...esto es incorrecto.
						if (importeNuevo2 == '0.00'){
							$("#importeAPagar").hide();
						}
					} else {
						document.getElementById("importeHTML").innerText = document.getElementById("importe").value;
...
// 					$('#datosDiscapacidad').removeClass('pae-hidden');
					$('#datosDiscapacidad').css('display','block');
					$("#datosDiscapacidad").addAttr("style").show();
				} else {
					document.getElementById("idComunidadDDExento").disabled = true;
					$("#oblComunidadDDExento").attr("style", "display:none;");
// 					$('#datosDiscapacidad').addClass('pae-hidden');
					$("#datosDiscapacidad").css("display", "none");
				}
			} else {
				$("#oblComunidadDDExento").attr("style", "display:none;");
// 				$('#datosDiscapacidad').addClass('pae-hidden');
	...
	function actualizarSolicitudPagada(idSeleccionado) {
		var iteracion = document.getElementsByName("checkbox-exemption");
				...
				if (document.getElementById("checkbox-exemption-" + j).checked) {
					...
					if (idSeleccionado == 1) {
						tooltip.removeClass().addClass(
										"pae-form__advisor-tooltip pae-form__advisor-tooltip--dynamic pae-form__advisor-tooltip--checkbox-one");
						tooltip.append($(".pae-form__advisor-checkbox--1")
								.html());
						if (document.getElementById("idComunidadDDExento") != null) {
							document.getElementById("idComunidadDDExento").disabled = false;
						}
	...
	function origenFirma() {
		...
		var str_comun = "JUSTIFICANTE:           "
				+ document.getElementById("numeroSolicitud").value + "\n"
				+ "NIF/CIF:                "
				+ document.getElementById("nif").outerText + "\n"
		...
		if (formPago == "C") {
			...
			var caducidad = document.getElementById("caducidadTarjeta1").value
					+ "/" + anyo.substr(2, 2);
...
<form:form modelAttribute="pagoSolicitudForm"
	action="/IPSC/secure/pagoSolicitud" id="formularioPago"
	name="pagoSolicitudForm" method="post" accept-charset="ISO-8859-1">
	<%
		if (!tipoPersona.equals("FH")) {
	%>
	<form:hidden path="nif" name="pagoSolicitud"
		value="${pagoSolicitud.nif}" />
	<form:hidden path="nombre" name="pagoSolicitud"
		value="${pagoSolicitud.nombre}" />
	<form:hidden path="apellido1" name="pagoSolicitud"
		value="${pagoSolicitud.apellido1}" />
	<form:hidden path="apellido2" name="pagoSolicitud"
		value="${pagoSolicitud.apellido2}" />
	<form:hidden path="numeroSolicitud" id="numeroSolicitud"
		name="pagoSolicitud" value="${pagoSolicitud.numeroSolicitud}" />
	<%
	...
	<form:hidden path="numeroSolicitud" id="numeroSolicitud"
		name="pagoSolicitud" value="${solicitud.numJustificante}" />
	<%
		}
	%>
	<form:hidden path="idSolicitudInicial"
		value="${altaSolicitud.idSolicitudInicial}" />
	...
	<form:hidden path="numSolicitud" id="numSolicitud"
		value="${altaSolicitud.numSolicitud}" />
	...
	<form:hidden name="altaSolicitud" id="inputIdFormaAcceso"
		path="idFormaAcceso" />

	<input type="hidden" id="comunidadesReqTitulo"
		value="${comunidadesReqTitulo}">
	...
	<input type="hidden" name="importe" id="importe"
		value="${Math.floor(registroSolicitud.importe)}" />
