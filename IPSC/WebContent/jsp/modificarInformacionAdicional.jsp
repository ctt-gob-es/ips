                  ...
                  <div class="pae-layout">
                    <div class="pae-layout__item pae-u-6/12 pae-u-6/12-lap pae-u-1/1-palm">
                      <div class="pae-form__cell">
                        ...
                        <form:select path="titulo" id="titulo" onchange="cambiarMayusculas(this)" class="pae-form__input" value="${altaSolicitud.titulo}">
                      ...
                      <div class="pae-form__cell pae-form__cell--hide">
                        ...
                        <c:choose> 
						    <c:when test="${solicitudForm.otrosTitulosList!= null && solicitudForm.otrosTitulosList.size() > 0}">
					       		<select name="otrosTitulos" id="otrosTitulos" class="pae-form__input pae-form__input--s-large" style="text-transform: uppercase;">
									<option value="0"><spring:message code="field.solicitud.selecciona" /></option>
									<c:forEach var="otrosTitulos" items="${solicitudForm.otrosTitulosList}">
										<c:choose>
									        <c:when test="${altaSolicitud.otrosTitulos eq otrosTitulos.descripcion}">
									        	<option value="${otrosTitulos.descripcion}" selected>${otrosTitulos.descripcion}</option>
									        </c:when>
									         <c:otherwise>
									         	<option value="${otrosTitulos.descripcion}">${otrosTitulos.descripcion}</option>
									         </c:otherwise>
									    </c:choose>     
									</c:forEach>
								</select>
		                  ...
		                  <div class="pae-layout">
							<logic:present name="listaPlantillasPropias" scope="request">
								<logic:iterate id="campo" name="listaPlantillasPropias" >					
									<logic:iterate id="campoValor" name="listaCamposPropios" >
										<c:if test="${campoValor.campo eq  campo.campoPropioBean.tituloCampo}">
											<div id="campoDinamico-${campo.campoPropioBean.id}" class="pae-layout__item pae-u-6/12 pae-u-6/12-lap pae-u-1/1-palm">
				                   				<div class="pae-form__cell" name="campPropio">
				                       				...
				                       				<c:choose>
													    	...
													    	<select id="plantilla${campo.campoPropioBean.id}" name="plantilla${campo.campoPropioBean.id}" class="pae-form__input" >
																<option value=""><spring:message code="field.solicitud.selecciona" /></option>
																 <c:forEach items="${campo.campoPropioBean.datosPropiosConfiguracion}" var="datoConf">
																 	<c:choose>
					           						    				<c:when test="${campoValor.valorVista eq datoConf.descripcion}">
					           						    					<option value="${datoConf.descripcion}" selected>${datoConf.descripcion}</option>
					           						    				</c:when>  
					           						    				<c:otherwise>
													    ...
													    <c:otherwise>
				                       						<input type="text" value="${campoValor.valorVista }" id="plantilla${campo.campoPropioBean.id}" name="plantilla${campo.campoPropioBean.id}" class="pae-form__input" style="text-transform: uppercase;" onpaste="comprobarPasteCaracteresEspeciales(this);">
													    </c:otherwise>
												    </c:choose>
