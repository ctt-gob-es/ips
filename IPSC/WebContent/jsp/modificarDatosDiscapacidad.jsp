...
<c:if test="${altaSolicitud.idMotivoReduccion eq 1}">
	<div class="animated fadeInLeft pae-layout__item pae-u-12/12 pae-u-12/12-lap pae-u-1/1-palm pae-margin-top"
		id="datosDiscapacidad">
</c:if>
<c:if test="${altaSolicitud.idMotivoReduccion ne 1}">
	<div class="animated fadeInLeft pae-layout__item pae-u-12/12 pae-u-12/12-lap pae-u-1/1-palm pae-margin-top"
		id="datosDiscapacidad" style="display: none">
</c:if>
	...
	<div class="pae-layout">
		<div class="pae-layout__item pae-u-2/12 pae-u-2/12-lap pae-u-1/3-palm">
			<div class="pae-form__cell">
				...
				<input type="text" name="porcentajeDiscapacidad" id="porcentajeDiscapacidad" value="${altaSolicitud.porcentajeDiscapacidad }"
					onchange="calcularImporte2();desmarcarChecks();" maxlength="3" value="" onkeypress="return soloNumeros(event);"
					onpaste="comprobarPasteCaracteresNumericos(this);"	title="<spring:message code="field.formulario790.convocatoria.tituloGradoDiscapacidad"/>"
		...
		<div class="pae-layout__item pae-u-6/12 pae-u-8/12-lap pae-u-1/1-palm">
			<div class="pae-form__cell">
				...
				<form:select path="idComunidadDDExento"	class="pae-form__input pae-form__select--disabled"	value="${altaSolicitud.idComunidadDDExento }"
					disabled="${altaSolicitud.porcentajeDiscapacidad lt 33}">
					...
					<c:forEach items="${listcomunidadesDiscapacidad}" var="comunidad">
						<c:choose>
							<c:when
								test="${comunidad.idComunidad eq altaSolicitud.idComunidadDDExento}">
		...
		<div class="pae-layout__item pae-u-1/1 pae-u-1/1-lap pae-u-1/1-palm">
			<div id="advisor-disabilities" class="pae-form__cell pae-hidden">
				<div class="pae-form__advisor-tooltip pae-form__advisor-tooltip--no-arrow pae-form__advisor-tooltip--no-shadow pae-form__advisor-tooltip--big">
					...
					<button name="advisor-disabilities-close"  id="advisor-disabilities-close" data-function="fc-advisor-close"
			...
			<div class="pae-form__cell">
				...
				<div class="pae-form__custom-rb-box">
					<c:if test="${altaSolicitud.tipoDiscapacidad eq null}">
						<input type="radio" name="tipoDiscapacidad"	path="tipoDiscapacidad" id="reservation-no" value="0"
							checked="checked" class="pae-form__custom-rb" onchange="cambioTipoDiscapacidad();" />
					</c:if>
					...
					<c:if test="${altaSolicitud.tipoDiscapacidad eq 1}">
						<input type="radio" name="tipoDiscapacidad"	path="tipoDiscapacidad" id="reservation-general" value="1"
							checked="checked" class="pae-form__custom-rb" onchange="cambioTipoDiscapacidad();" />
					</c:if>
					...
					<c:if test="${altaSolicitud.tipoDiscapacidad eq 2}">
						<input type="radio" name="tipoDiscapacidad"	path="tipoDiscapacidad" id="reservation-intellectual" value="2"
							checked="checked" class="pae-form__custom-rb" onchange="cambioTipoDiscapacidad();" />
					</c:if>
			...
			<div class="pae-form__cell">
				...
				<c:choose>
					<c:when	test="${solicitudForm.discapacidadList != null && solicitudForm.discapacidadList.size() > 0}">
						<select name="detalleDiscapacidad" id="detalleDiscapacidad"	class="pae-form__input" 
					...
					<c:otherwise>
						<c:choose>
							<c:when test="${altaSolicitud.porcentajeDiscapacidad > 0 && altaSolicitud.porcentajeDiscapacidad <= 100}">
								<input type="text" name="detalleDiscapacidad"  id="detalleDiscapacidad"
									value="${altaSolicitud.detalleDiscapacidad}"
									class="pae-form__input"	onchange="return cambiarMayusculasText(this);"
									style="text-transform: uppercase;" maxlength="200">
							</c:when>
							<c:otherwise>
								<input type="text" name="detalleDiscapacidad" id="detalleDiscapacidad"
									value="${altaSolicitud.detalleDiscapacidad}"
									class="pae-form__input" disabled="true"	onchange="return cambiarMayusculasText(this);"
									style="text-transform: uppercase;" maxlength="200">
							</c:otherwise>
						</c:choose>
					</c:otherwise>
