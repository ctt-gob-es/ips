				...
				if(document.getElementById("checkbox-exemption-"+j).checked) {
					...
					for(var i=1;i<=orgsSelect.length;i++){
						...
						if(id==idSeleccionado){
							...
							var listImportes = document.getElementsByName("importe");
							var k;
							for(k = 0; k < listImportes.length ; k++){
								var importe = listImportes[k];
								importe.value = importeNuevo;
							}
					...
					document.getElementById("importe").value = parseFloat(($('#importeOriginal').val() - $('#importePagado').val())).toFixed(2);
					document.getElementById("importeHTML").innerText = document.getElementById("importe").value;
					if(document.getElementById("importe_2"))
...
<head>
	<link rel="stylesheet" href="../css/efectoBarrido.css">
	<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1">
		...
		<form:form modelAttribute="pagoSolicitudForm" action="/IPSC/secure/pagoSolicitud" id="formularioPago" name="pagoSolicitudForm" method="post" accept-charset="ISO-8859-1">
			...
			<input type="hidden" name="motivoTarjeta" id="motivoTarjeta" value="0" />
      		...
      		<input type="hidden" name="importePagado" id="importePagado" value="${solicitudForm.importePagado }"/>
      		<input type="hidden" name="importeOriginal" id="importeOriginal" value="${solicitudForm.importeOriginal }"/>
      		<input type="hidden" name="porcentajeMinDiscapacidad" id="porcentajeMinDiscapacidad"  value="${solicitudForm.porcentajeMinDiscapacidad }"/>
			
			<%@ include file="/jsp/datosExencion.jsp" %>
			<%@ include file="/jsp/datosPago.jsp" %>
