...
public class BuscarConvocatoriasSpring extends AbstractSpring {
	...
	protected String doExecute(SpringForm form) throws Exception {
		logger.debug("BuscarConvocatoriasSpring -start");
		...
		if (!"".equals(formulario.getSearch()) && formulario.getSearch() != null){
			...
			if(centroGestorAuxDescripcion.size() > 0){
				...
				for(int i=0; i<ListaCentroGestor.size(); i++) {
						if(ListaCentroGestor.get(i).getDescripcion().contains(formulario.getSearch())) {
				...
				if(centroGestorAuxSiglas.size() > 0){
					...
					for(int i=0; i<ListaSiglasCentroGestor.size(); i++) {
						if(ListaSiglasCentroGestor.get(i).getDescripcion().contains(formulario.getSearch())) {
			...
			try {
				...
				if (cuerpoEscalaAuxDescripcion.size() > 0){
					...
					for(int i=0; i<ListaCuerpoEscala.size(); i++) {
						if(ListaCuerpoEscala.get(i).getDescripcion().contains(formulario.getSearch())) {
		...
		convocatorias = convocatoriasManager.buscarConvocatoriasView(convocatoriaQuery);
		

		
		if(error == 1) {
			this.setRequestAttribute("numTotalConvocatorias", 0);
			this.setRequestAttribute("numTotalConvocatoriasSub", 0);
			return "success";
		}
		...
		if (verTodo && convocatorias != null && convocatorias.size()!= 0){
			this.setRequestAttribute("verTodo", 0);
			this.setRequestAttribute("numTotalConvocatorias", convocatorias.get(0).getNumTotal());
		}else{
			this.setRequestAttribute("verTodo", 1);
			this.setRequestAttribute("numTotalConvocatorias", convocatorias.size());
		}
	 ...
	 * @return el convocatorias abiertas view query
	 */
	private ConvocatoriasAbiertasViewQuery crearQueryConvocatoria(BuscaConvocatoriasForm formulario,String numReg,String busqueda) {
		...
		ConvocatoriasAbiertasViewQuery convocatoriaQuery = new ConvocatoriasAbiertasViewQuery();
	...
	private SolComunViewQuery crearQuerySolicitud(BuscaConvocatoriasForm formulario,String numReg,String busqueda) {
		...
		solicitudComunQuery.setMaxResults(Integer.parseInt(Constantes.NUMERO_REGISTROS_TOTALES));
