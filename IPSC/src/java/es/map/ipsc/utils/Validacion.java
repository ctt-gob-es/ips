...
public class Validacion {
	...
	/** La constante STRING_PATTERN. */
	private static final String STRING_PATTERN = "[^a-zA-Z0-9áéíóúÁÉÍÓàèìòùÀÈÌÒÙâêîôûÂÊÎÔÛ/ÚÑñäÄëËïÏöÖüÜç¿!¡'Ç\"%·´>_.,;:\\?\\-\\(\\)\\ª\\º\\s\\+\\*]+";
	
	/** La constante STRING_LETRAS. */
	private static final String STRING_LETRAS = "TRWAGMYFPDXBNJZSQVHLCKE";
	...
	public boolean nifValidate(String nif) {
        ...
        if(valor.length() == 9){
                      ...
                      //1. DNI: 8 numeros + digito de control
                  	...
                  	//2. NIF K,M o L: [K,M,L] + 7 numeros + digito de control
                  	mask = Pattern.compile("[KML][0-9]{7}[ABCDEFGHJKLMNPQRSTVWXYZ]{1}");
                  	matcher = mask.matcher(valor);
        
                  	if(matcher.matches()){
                       tipoNIF = 2;
                  	} 
        
                  	//Validacion del DNI, NIF por digito de control
                  	if(tipoNIF == 1){
	                     posicion_letra = Integer.parseInt(valor.substring(0,8))%23;
	                     String digitoControl = valor.substring(8,9);
	                     String letras = STRING_LETRAS;
                       ...
                       posicion_letra = Integer.parseInt(valor.substring(1,8))%23;
                         String digitoControl = valor.substring(8,9);
                         String letras = STRING_LETRAS;
                         String digitoControlCalculado = letras.substring(posicion_letra,posicion_letra+1);
	 ...
	 * Patron: [^a-zA-Z0-9áéíóúÁÉÍÓÚÑñäÄëËïÏöÖüÜçÇ'.,:\\-\\ª\\º\\s]+
	 *
	 * @param cadena el cadena
	 * @return verdadero, si tiene exito
	 */
	...
	public boolean validateApellido(String cadena) {
		...
		//Pattern p = Pattern.compile("[^a-zA-Z0-9áéíóúÁÉÍÓàèìòùÀÈÌÒÙâêîôûÂÊÎÔÛ/ÚÑñüÜç¿!¡'Ç\"%·´>_.,;:\\?\\-\\(\\)\\ª\\º\\s\\+\\*]+");
		Pattern p = Pattern.compile("[^a-zA-Z0-9áéíóúÁÉÍÓàèìòùÀÈÌÒÙâêîôûÂÊÎÔÛ/ÚÑñäÄëËïÏöÖüÜç¿!¡'Ç\"%·´>_.,;:\\?\\-\\(\\)\\ª\\º\\s\\+\\*]+");
		Matcher m = p.matcher(cadena);
	...
	public boolean validateNameDoc(String cadena) {
		...
		Pattern p = Pattern.compile("[^a-zA-Z0-9áéíóúÁÉÍÓ/ÚÑñüÜç¿!¡Ç'\"_.,:\\?\\-\\(\\)\\ª\\º\\s]+");
		Matcher m = p.matcher(cadena);
    ...
    public boolean esUnaDireccionIPValida( String dir ) {
        ...
        while ( st.hasMoreTokens() ) {
            ...
            try {
            	...
            	logger.error("error parsear nroInt: "+ nroInt,s);
                return false;
            }
	...
	public boolean isNumeric (String cadena){
		...
		try {		
			...
			logger.error("error parsear cadena "+ cadena,nfe);
			return false;		
		}	
	}
