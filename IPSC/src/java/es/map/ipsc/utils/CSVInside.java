...
import es.mpt.dsic.inside.ws.service.*;
...
public class CSVInside {
	...
	public static String generarCodigoCSV(byte[] byteJutificante, Properties csvProperties, EeUtilService csvEeUtilService) {
		...
		try {
			...
			logger.info("Error en la generacion del CSV",re);
		
		} catch (WebServiceException we) {
			logger.info("Error en la generacion del CSV",we);
		}
		return codigoCSV;
	}
	...
	public static byte[] generarCopiaCSV(byte[] bytePDF, CiudadanoBean usuarioActual, Properties csvProperties,
		...
		try {
			...
			logger.info("Error en la generacion del Justificante " + codigoCSV + " : ",we);
		} catch (Exception e) {
			logger.info("Error en la generacion del Justificante " + codigoCSV + " : ",e);
		} 
